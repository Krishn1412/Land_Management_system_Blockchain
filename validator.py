...
mph = {}
class Transaction:
    def _init_(self, buyer_name,seller_name,prop_obj,amount):
...
class Block:
    def _init_ (self,prev_hash,merkleRootHash,timestamp):
...
class Property:
    def _init_(self,currentOwner,id,PropertyName,PreviousTransacs,sizep):
...
def winner(list_of_per):
    ...
    return st[n-1][0]
class person:
    def _init_(self,id,name,prop,total):
        self.id=id
        self.name=name
        self.prop=prop
        self.total=total
